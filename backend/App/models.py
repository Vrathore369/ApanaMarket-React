...
class Product (models.Model):
    ...
    def update_is_in_cart(self, user):
        if user.is_authenticated:
            cart_product_ids = Cart.objects.filter(user=user, product=self).exists()
            self.is_in_cart = cart_product_ids
            self.save()
    ...
    def serialize(self):
        return {
            ...
            'type': self.type,
            'product_image': str(self.product_image.url),
            'offer_1': self.offer_1,
            'offer_2': self.offer_2,
            'offer_3': self.offer_3,
            'offer_4': self.offer_4,
        }
